...
Forge Blaze Ignite（FBI）是一套模块化、类型安全、可测试的游戏开发框架。纯 TypeScript 编写，框架层零引擎依赖——通过 Runtime 适配层桥接任意游戏引擎。
...
- **Logger** — 分级日志系统（Debug/Info/Warn/Error），支持 Tag 过滤和自定义输出目标
...
- **[fbi-ecs](packages/gfc-ecs/README.md)** — 高性能 ECS 插件，基于 SparseSet 架构，支持 Generational ID、查询缓存和响应式分组
- **fbi-timer-heap** — 最小堆定时器实现，适用于定时任务数量中等的场景
- **fbi-timer-wheel** — 时间轮定时器实现，适用于海量定时任务的高吞吐场景
...
packages/
├── gfc-ecs/          # ECS 插件
├── gfc-timer-heap/   # 最小堆定时器
└── gfc-timer-wheel/  # 时间轮定时器
```
