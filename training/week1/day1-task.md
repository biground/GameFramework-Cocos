...
### 任务 1：实现 `ModuleBase` 抽象类

**文件路径**：`assets/scripts/framework/core/ModuleBase.ts`
...
### 任务 2：实现 `GameModule` 模块管理器

**文件路径**：`assets/scripts/framework/core/GameModule.ts`
...
### 任务 3：实现 `GameEntry` 门面类

**文件路径**：`assets/scripts/framework/core/GameEntry.ts`
...
**文件路径**：`tests/core/game-module.test.ts`
...
完成编码后，请思考并回答以下问题（在 Copilot Chat 中回答，我会评价）：
...
- [ ] `ModuleBase.ts` 实现完成，有中文 JSDoc 注释
...
- [ ] 代码中没有 `any` 类型
- [ ] 代码中没有 `cc` 命名空间引用
- [ ] `git commit` 使用规范格式（如 `feat(core): 实现 GameEntry 和 ModuleBase`）
